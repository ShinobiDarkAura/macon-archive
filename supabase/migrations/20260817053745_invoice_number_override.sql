-- Allow a hand-written invoice number.
--
-- The sequence still advances so seq stays unique per kind, but if a number is
-- supplied it is used verbatim. A clash trips the existing unique index rather
-- than silently overwriting.
create or replace function public.new_invoice(p jsonb)
returns public.invoices
language plpgsql security definer set search_path = public
as $$
declare n integer; pre text; rec public.invoices;
begin
  update public.invoice_counters set last_seq = last_seq + 1
   where kind = p->>'kind' returning last_seq into n;
  if n is null then raise exception 'unknown invoice kind: %', p->>'kind'; end if;
  ...
  values
    (p->>'kind', n,
     coalesce(nullif(trim(p->>'number'), ''), pre || '-' || lpad(n::text, 3, '0')),
